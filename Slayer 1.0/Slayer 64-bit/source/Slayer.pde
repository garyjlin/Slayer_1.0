...
gravity Gjump = new gravity(-9.8, 3, 100); // change value for jump height
gravity Gfall = new gravity(0.1, 0, 0);
   ...
   if (attackAnimation == true){
     if (charLeft == true){   
       ...
       if(tAttackAnimation.getTime() >=600){
         ...
         charImg = charStand[int(charLeft)];
       ...
       if(tAttackAnimation.getTime() >=600){
         charAttack = charStand[int(!charLeft)];
         enemyAlreadyDamaged = false;
         tAttackAnimation.stop();
         attackAnimation = false;
       }else{
...
void animate(){
  //Running
   ...
   //Standing
   if(run[0] == false && run[1] == false)charImg = charStand[int(charLeft)]; 
   else if(fall == false)
   {
...
void mousePressed(){
  if (attackAnimation ==false){
    ...
    if (charLeft == true){ // left      
      attackLeft = true;  
      attackBox.x = MC.x - MC.w-(int)1.3*MC.w; 
      attackBox.w = MC.w + (int)1.3*MC.w;      
      attackBox.y = MC.y;
    } else { // right
      attackRight = true;
